...
def select_manifest_rows(config: V01TrainConfig) -> tuple[list[dict], list[dict]]:
    ...
    train: list[dict] = []
    dev: list[dict] = []
    for corpus in ("waxal", "gnlp"):
        corpus_train = sorted(
            [
                row
                for row in rows
                if row["corpus"] == corpus and row["split"] == "train"
            ],
            key=lambda row: hashlib.sha256(
                f"{config.seed}:{config.run_name}:train:{row['record_id']}".encode()
            ).hexdigest(),
        )[: config.train_per_corpus]
        dev_split = "dev" if corpus == "waxal" else "validation"
        corpus_dev = sorted(
            [
                row
                for row in rows
                if row["corpus"] == corpus and row["split"] == dev_split
            ],
            key=lambda row: hashlib.sha256(
...
def load_source_dataset(corpus: str, split: str):
    ...
    if corpus == "gnlp":
        # Avoid the original builder because it generates every split, including
        # a large unlabeled split, even when we only need indexed train rows.
        return load_dataset(
            "parquet",
            data_files=list(GNLP_TRAIN_PARQUET),
            split="train",
            cache_dir=CACHE_DIR,
        ).cast_column("audio", Audio(sampling_rate=16000))

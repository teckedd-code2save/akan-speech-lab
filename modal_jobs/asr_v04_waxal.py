...
APP_NAME = "akan-speech-asr-v04-waxal"
...
class V01TrainConfig:
    ...
    train_per_corpus: int = 9138
    eval_per_corpus: int = 1024
...
def select_manifest_rows(config: V01TrainConfig) -> tuple[list[dict], list[dict]]:
    ...
    rows = read_manifest()
    train = sorted(
        [row for row in rows if row["corpus"] == "waxal" and row["split"] == "train"],
    ...
    if not train or not dev:
        raise RuntimeError("v0.4 Waxal-only train/dev row selection is empty")
    return train, dev
...
This is a Ghanaian Speech Lab ASR artifact: a Waxal-only low-learning-rate
...
""",
    ...
    commit = api.upload_folder(
        ...
        commit_message="Publish Waxal-only low-LR GSL ASR v0.4 checkpoint",

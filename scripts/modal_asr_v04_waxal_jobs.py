...
from modal_jobs.asr_v04_waxal import APP_NAME, CODE_NAME, V01TrainConfig  # noqa: E402
...
def deploy() -> dict[str, Any]:
    proc = subprocess.run(
        ["modal", "deploy", "modal_jobs/asr_v04_waxal.py"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Durable Modal controller for the Waxal-only GSL ASR run")

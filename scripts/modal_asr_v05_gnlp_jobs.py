...
from modal_jobs.asr_v05_gnlp import APP_NAME, CODE_NAME, V01TrainConfig  # noqa: E402
...
def deploy() -> dict[str, Any]:
    proc = subprocess.run(
        ["modal", "deploy", "modal_jobs/asr_v05_gnlp.py"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def submit() -> dict[str, Any]:
    ...
    if state.get("deployment") != "ready":
        raise RuntimeError("Deploy the ASR app before submitting training")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Durable Modal controller for the GhanaNLP-only GSL ASR run")
